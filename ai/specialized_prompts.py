"""
FundPilot-AI 专业化 AI 决策 Prompt（v2.0 - 开放式分析）
...
def _get_gold_etf_prompt() -> str:
    ...
    return """你是一位拥有20年贵金属投资经验的资深投资顾问。
...
def _get_commodity_cycle_prompt() -> str:
    ...
    return """你是一位深耕大宗商品和周期股投资的资深投资顾问，拥有丰富的周期投资经验。
...
def _get_default_etf_prompt() -> str:
    """默认ETF Prompt - 通用分析"""
    return """你是一位经验丰富的基金投资顾问，擅长基于量化数据进行投资决策分析。
...
def _get_default_bond_prompt() -> str:
    """默认债券 Prompt - 通用分析"""
    return """你是一位经验丰富的固定收益投资顾问，擅长债券基金的投资决策分析。
