...
class TestPercentile60:
    """60日分位值计算测试"""
    
    def test_middle_value(self):
        """测试中间值"""
        prices = [1.0, 1.5, 2.0]  # min=1, max=2
        result = calculate_percentile_60(1.5, prices)
        assert result == 50.0
    
    def test_min_value(self):
        """测试最小值"""
        prices = [1.0, 1.5, 2.0]
        result = calculate_percentile_60(1.0, prices)
        assert result == 0.0
    
    def test_max_value(self):
        """测试最大值"""
        prices = [1.0, 1.5, 2.0]
        result = calculate_percentile_60(2.0, prices)
        assert result == 100.0
    
    def test_below_min(self):
        """测试低于最小值（限制在 0）"""
        prices = [1.0, 1.5, 2.0]
        result = calculate_percentile_60(0.5, prices)
        assert result == 0.0
    
    def test_above_max(self):
        """测试高于最大值（限制在 100）"""
        prices = [1.0, 1.5, 2.0]
        result = calculate_percentile_60(2.5, prices)
        assert result == 100.0
    ...
    def test_empty_prices(self):
        """测试空列表"""
        result = calculate_percentile_60(1.0, [])
        assert result == 50.0
...
class TestMA60:
    ...
    def test_normal(self):
        """测试正常计算"""
        prices = [1.0, 2.0, 3.0]
        result = calculate_ma_60(prices)
        assert result == 2.0
    
    def test_empty(self):
        """测试空列表"""
        result = calculate_ma_60([])
        assert result == 0.0
...
class TestCalculateAllMetrics:
    ...
    def test_full_metrics(self):
        """测试完整指标计算"""
        prices = [1.0, 1.1, 1.2, 0.9, 1.05]  # 60 天模拟
        metrics = calculate_all_metrics(1.15, prices, daily_change=2.5)
        
        assert metrics.max_60 == 1.2
        assert metrics.min_60 == 0.9
